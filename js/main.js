...
let houseName= '', newSource='', dataRef, targetSource;

function changeHouse()
{
  dataRef= this.dataset.offset;

  let houseName = this.className.split(" ")[1];
  let newSource = houseName.charAt(0).toUpperCase() + houseName.slice(1);

  currentHouseName.textContent = `House Of ${houseData[dataRef][0]}`;
  houseDescription.textContent = `${houseData[dataRef][1]}`;
...
sigilButtons.forEach(button => button.addEventListener("click", changeHouse));
